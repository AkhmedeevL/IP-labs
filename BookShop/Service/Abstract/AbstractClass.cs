...
{
   abstract public class AbstractClass : IBookShop

     {
        ...
        public BaseEntity findById(int? id) {
         ...
         public List<BaseEntity> getList() {
            ...
            List<BaseEntity> CategoryList = new List<BaseEntity>();
            foreach (string item in filesPaths)
            {
                StreamReader stream = new StreamReader(item, true);
                BaseEntity Category = (BaseEntity)xsSubmit.Deserialize(stream);
